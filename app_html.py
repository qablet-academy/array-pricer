"""ArrayPricer for Bonds using Html Table"""
...
def add_bond(n_clicks, rows):
    ...
    new_row = html.Tr(
        [
            ...
            html.Td(
                dcc.Input(
                    id={"type": "coupon-input", "index": new_index},
                    type="number",
                    placeholder="Coupon (%)",
                )
            ),
            ...
            html.Td(
                dcc.Input(
                    id={"type": "frequency-input", "index": new_index},
                    type="text",
                    placeholder="Frequency",
                )
            ),
            html.Td(
                dcc.Input(
                    id={"type": "notional-input", "index": new_index},
                    type="number",
                    value=100,
                    placeholder="Notional",
                )
...
@app.callback(
    ...
    [
        ...
        Input({"type": "notional-input", "index": dash.ALL}, "value"),
    ],
)
def update_price(
    coupons, accrual_starts, maturities, frequencies, currencies, notionals
):
    prices = []
    ...
    for i in range(max_len):
        ...
        notional = notionals[i] if i < len(notionals) else None

        # Skip processing if any value is missing
        if None in [coupon, accrual_start, maturity, frequency, currency, notional]:
        ...
        prices.append(f"${final_price:.6f}")
