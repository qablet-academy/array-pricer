...
from app_aggrid import show_timetable,update_rate_graph
...
def test_delete_bond():
    ...
    def run_callback():
        context_value.set(AttributeDict(**{"triggered_inputs": [{"prop_id": "bond-table.cellRendererData"}]}))
        return update_bond_data(None, None, {"value": 1, "rowIndex": 0}, None, pricing_datetime, initial_data)
...
def test_rate_graph_update():
    ...
    assert len(figure['data'][0]['x']) == len(figure['data'][0]['y'])  # X and Y data must match in length